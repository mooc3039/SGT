      ...
      <div class="panel-body">
        ...
        <div class="row">
          <div class="col-md-12">
            ...
            <table class="table table-striped table-advance table-hover" id="tbl_index_pagamentos_entrada" data-order='[[ 3, "desc" ]]'>
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td>
                    {{ number_format($pagamento_entrada->valor_pago, 2, '.', ',')}}
                  </td>
                  <td>
                    {{ date('d-m-Y H:m:s', strtotime($pagamento_entrada->created_at)) }}
                  </td>
                  <td>
                    {{ date('d-m-Y H:m:s'), strtotime($pagamento_entrada->updated_at)}}
                  </td>
                </tr>
                @endif
                @endforeach
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    ...
    var titulo = "Pagamentos da Entrada "+codigo_entrada;   
    var msg_bottom = "Papelaria Agenda & Serviços";

    var oTable = $('#tbl_index_pagamentos_entrada').DataTable( {
      "processing": true,
  ...
  $(document).ready(function(){
    ...
    var valor_total_iva_visual = Number.parseFloat($('#valor_total').val()); // Atencao: nao existe iva, erro de expressao que ja nao era possivel eliminar devido ao estado avancado do codigo
    ...
    var pago = $('#pago').val();
    var pago_total_iva_info = Number.parseFloat($('#pago_total_iva_info').val());
    var valor_pago_soma = Number.parseFloat($('#valor_pago_soma').val());
  ...
  function resetPagamento(){
     ...
     $('#remanescente').val(Number.parseFloat($('#valor_remanescente_ref').val()).formatMoney());
     $('#valor_pago').val(0);
   }
